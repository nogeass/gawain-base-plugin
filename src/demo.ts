#!/usr/bin/env node
/**
 * Demo CLI for gawain-base-plugin
...
import { convertBaseProduct, validateBaseProduct } from './platform/base_adapter.js';
...
async function main(): Promise<void> {
  console.info('=== Gawain BASE Plugin Demo ===\n');
  ...
  if (!validateBaseProduct(rawProduct)) {
    console.error('Invalid product format. Expected BASE product structure.');
    process.exit(1);
  }

  const productInput = convertBaseProduct(rawProduct);
